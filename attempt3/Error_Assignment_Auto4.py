...
dbscan_eps = 0.0095            # DBSCAN eps parameter – smaller values tend to break clusters apart
...
# Instead of targeting a specific number of clusters, we now search for the threshold that gives the best clustering quality,
# as measured by the silhouette coefficient.
best_threshold = None
best_silhouette = -1.0
...
for candidate_threshold in np.arange(0.005, 0.007, 0.0001):
    ...
    # Only proceed if there are candidate points.
    ...
    # We can only compute a silhouette score if there are at least 2 clusters.
...
def minimal_bounding_sphere(points):
    ...
    p2 = points[j]
    # Initial sphere: centre is the midpoint of p1 and p2; radius is half the distance between them.
    centre = (p1 + p2) / 2.0
