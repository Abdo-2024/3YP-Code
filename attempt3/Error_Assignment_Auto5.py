...
# Instead of targeting a fixed cluster count, we choose the threshold that gives the best silhouette score.
best_threshold = None
best_silhouette = -1.0
...
# ---------------------------
# Merging Overlapping Spheres
# ---------------------------
def merge_overlapping_spheres(spheres):
    """
    Merge any number of overlapping spheres into a single sphere.
    ...
    For each connected group of overlapping spheres, the new sphere has:
      - Centre: the average of the centres.
      - Radius: (min(radius)/2 + max(radius)) over the group.
      - Deformation intensity: the average intensity.
      - Colour: determined from the new radius.
    """
    n = len(spheres)
    ...
    def find(i):
        # Find the representative of sphere i
        while parent[i] != i:
    ...
    def union(i, j):
        # Merge the sets of sphere i and sphere j
        ri = find(i)
        rj = find(j)
        if ri != rj:
            parent[rj] = ri

    # Build the overlap graph: if two spheres overlap, union them.
    for i in range(n):
    ...
    # Group spheres by their connected component (i.e. same representative)
    groups = {}
    for i in range(n):
        root = find(i)
    ...
    for group in groups.values():
        # Compute the new centre as the average of the centres.
        centres = np.array([s['centre'] for s in group])
        new_centre = np.mean(centres, axis=0)
        
        # Determine the new radius using the smallest and largest radii.
        radii = np.array([s['radius'] for s in group])
        new_radius = (np.min(radii) / 2.5) + (np.max(radii)/1.2)
        ...
        # Recompute the colour based on the new radius.
        if new_radius > 0.01:
