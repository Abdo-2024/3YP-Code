...
import numpy as np
import csv
from scipy.spatial import cKDTree  # Fast, vectorised nearest-neighbour queries
...
- adaptive_multiplier: Multiplier for adaptive threshold calculation.
...
# ---------------------------
# User-defined parameters – these are now more data-driven
# ---------------------------
n_points = 50000
threshold_icp = 0.0002  # ICP alignment distance threshold

# Adaptive threshold multiplier (the threshold will be set to mean + multiplier*std)
...
# Ensure the meshes have vertex normals (improves ICP registration and visualisation)
...
# Calculate an adaptive threshold using the mean and standard deviation
mean_dist = np.mean(distances)
std_dist = np.std(distances)
adaptive_threshold = mean_dist + adaptive_multiplier * std_dist
print("Adaptive threshold: {:.8f}".format(adaptive_threshold))

# Use a Boolean mask to select deformation points
...
if len(deformation_points) > 0:
    ...
    print("No deformation points detected – consider lowering the multiplier or reviewing your data.")

# ---------------------------
# Step 4: Cluster the Deformation Points using HDBSCAN (Automatic clustering)
# ---------------------------
# HDBSCAN automatically detects clusters based on the density; you only need to set the minimum cluster size.
clusterer = hdbscan.HDBSCAN(min_cluster_size=5)
labels = clusterer.fit_predict(deformation_points)
unique_labels = set(labels)
if -1 in unique_labels:
    unique_labels.remove(-1)
print("Detected clusters (blobs):", len(unique_labels))
if len(unique_labels) == 0:
    print("No clusters detected – consider adjusting HDBSCAN parameters.")
...
def minimal_bounding_sphere(points):
    ...
    # Pick an arbitrary point p0 and find the point p1 farthest from p0
    p0 = points[0]
    ...
    # Enlarge the sphere iteratively to enclose all points using vectorised distance calculations
    while True:
...
for label in unique_labels:
    ...
    # Compute the residual errors (differences between the actual distances and the sphere's radius)
    ...
    # Colour assignment based on the sphere's radius
    if radius > threshold_high:
        colour = [0.5, 0, 0.5]    # Purple for larger errors
    elif radius > (threshold_low + threshold_high) / 2:
        colour = [1, 0, 0]        # Red for intermediate errors
    elif radius > threshold_low:
        colour = [1, 0.65, 0]     # Orange for small errors
    else:
        colour = [1, 1, 0]        # Yellow for very subtle errors
    ...
    print(f"Blob {label}: Centre = {centre}, Radius = {radius:.6f}, Deformation Intensity = {deformation_intensity:.6f}")
...
with open(csv_filename, 'w', newline='') as csvfile:
    ...
    for row in csv_data:
        writer.writerow(row)
print("Sphere data written to", csv_filename)
