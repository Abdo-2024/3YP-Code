...
# For the new objective lens, the object-side NA is specified as 0.04 (in air)
NA_object_fixed = 0.02  
# We assume the sample has a refractive index:
n_sample = 1.52

# --- Laser Specs and Build Volume ---
# Laser bandwidth (difference between max and min wavelengths)
laser_bandwidth = lambda0_max - lambda0_min  # in meters
# Build volume dimensions (in meters)
build_volume_x = 3.6e-3  # 3.6 mm
build_volume_y = 3.6e-3  # 3.6 mm
build_volume_z = 500e-6  # 500 µm

# OCT scanning volume depth (axial thickness per volume)
oct_volume_thickness = 20e-6  # 20 µm
# Total allowed imaging time for full build volume (in seconds)
allowed_time = 80  # seconds
...
# We simulate a narrow range around the fixed object NA (0.03 to 0.07)
num_NA = 1000
NA_values = np.linspace(0.02, 0.07, num_NA)  # object-side NA values (in air)
# Compute effective NA in the sample:
NA_sample_values = NA_values * (n_sample / n_air)  # Multiply by 1.52

# --- Target Depth of Focus (DOF) ---
target_DOF = 100  # desired DOF in micrometers
...
# --- OCT Detector Calculations ---
# Compute the number of lateral pixels required.
# We assume the effective pixel size should match the optimal lateral resolution.
# Convert the build volume lateral dimensions from meters to µm:
num_pixels_x = int(np.round((build_volume_x * 1e6) / optimal_lat_res))
num_pixels_y = int(np.round((build_volume_y * 1e6) / optimal_lat_res))
lateral_pixel_size = build_volume_x / num_pixels_x  # in meters

# Compute the number of volumes (or z steps) required to cover the build volume:
num_volumes = int(np.ceil(build_volume_z / oct_volume_thickness))

# Total number of A-scans required = (number of volumes) x (pixels per lateral scan)
total_A_scans = num_volumes * num_pixels_x * num_pixels_y

# Required detector line rate (A-scans per second)
line_rate = total_A_scans / allowed_time

print("\nOCT Detector Requirements:")
print(f"  Lateral pixel count: {num_pixels_x} x {num_pixels_y}")
print(f"  Effective lateral pixel size: {lateral_pixel_size*1e6:.2f} µm")
print(f"  Number of volumes (z steps): {num_volumes}")
print(f"  Total A-scans needed: {total_A_scans:.0f}")
print(f"  Required line rate: {line_rate/1e3:.1f} kHz")
...
# Optionally, you can still display the plot with plt.show() if needed:
plt.show()
