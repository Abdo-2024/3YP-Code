...
const ShaderReplacementRule MESH_PROPAGATE_GWN_VALUE (
    /* rule name */ "MESH_PROPAGATE_GWN_VALUE",
    { /* replacement sources */
      {"VERT_DECLARATIONS", R"(
          in float a_value;
          in vec3 a_bcoords;
          out float a_valueToFrag;
          out vec3 a_bcoords2frag;
        )"},
      {"VERT_ASSIGNMENTS", R"(
          a_bcoords2frag = a_bcoords;
          a_valueToFrag = a_value;
          // a_valueToFrag = a_value / (a_bcoords.x + a_bcoords.y + a_bcoords.z);
        )"},
      {"FRAG_DECLARATIONS", R"(
          in float a_valueToFrag;
          in vec3 a_bcoords2frag;
        )"},
      {"GENERATE_SHADE_VALUE", R"(
          float shadeValue = a_valueToFrag / (a_bcoords2frag.x + a_bcoords2frag.y + a_bcoords2frag.z);
          //float shadeValue = a_valueToFrag;
        )"},
    },
    /* uniforms */ {},
    /* attributes */ {
      {"a_value", DataType::Float},
      {"a_bcoords", DataType::Vector3Float},
    },
    /* textures */ {}
);
